...
class TestElements:
    ...
    class TestWebTable:
        ...
        def test_web_table_update_person_info(self, driver):  # доделать код, тк изменяет ток одно поле
            web_table_page = WebTablePage(driver, 'https://demoqa.com/webtables')
            web_table_page.open()
            second_name = web_table_page.add_new_person()[1]
            web_table_page.search_some_person(second_name)
        ...
        def test_web_table_change_count_row(self, driver):
            ...
            assert count == [5, 10, 20, 25, 50,
                             100], "the numbers of rows in the table has not been changed or has changed incorrectly"
    ...
    class TestLinkPage:  # Тест с норм ссылкой дропается. Нужно чинить
